...
-- captures any symbol
local symbol = whitespace * p.apply(p.capture(p.one_or_more(
  p.either(
    p.range('az'),
    p.range('AZ'),
    p.one_of('-_?!')
...
local number = whitespace * p.apply(p.apply(p.capture(p.one_or_more(p.range('09'))), tonumber), types.make_number)
...
local moonlisp_parser = p.grammer {
  ...
  list = startings * (
      p.capture(p.zero_or_more(p.either(number, symbol)))
      ) 
    * endings
}
